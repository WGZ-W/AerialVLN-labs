...
class IWTrajectoryDataset(torch.utils.data.IterableDataset):
    ...
    def __next__(self):
        ...
        # 假设已有 image_processor 和 tokenizer
        pixel_values = image_processor(obs['rgb'].numpy(), return_tensors='pt')['pixel_values'].squeeze(0)
        input_ids = tokenizer(instruction_text, return_tensors='pt')['input_ids'].squeeze(0)
...
def initialize_trainer(dagger_it=0, checkpoint_folder=None):
    ...
    policy = LLaVAPolicy(config, action_space)
    trainer = VLNCETrainer(
        load_from_ckpt=False,
        observation_space=observation_space,
        action_space=action_space,
        policy_net=policy,  # 新增参数或修改原有
    )
    return trainer
