...
class DDPIWTrajectoryDataset(torch.utils.data.IterableDataset):
        ...
        if len(self._preload) == 0:
            ...
            with lmdb.open(
                ...
                for i in range(self.preload_size):
                    ...
                    try:
                        data = custom_msgpack.unpackb(value)  # 使用自定义 unpackb
    ...
    def __next__(self):
        ...
        # --- 预处理图像 ---
        rgb_frames = obs['rgb']
        print(f"Type of rgb_frames: {type(rgb_frames)}")
...
class IWTrajectoryDataset(torch.utils.data.IterableDataset):
    ...
    def __next__(self):
        ...
        # --- 预处理图像 ---
        # obs['rgb'] 形状: [T, H, W, 3], dtype=uint8
        rgb_frames = obs['rgb']  # numpy array
        # 转换为 PIL 图像列表
        from PIL import Image
        pil_images = [Image.fromarray(frame) for frame in rgb_frames]
        # 批处理所有帧
        pixel_values = self.image_processor(pil_images, return_tensors='pt')['pixel_values']  # [T, C, H, W]
        obs['pixel_values'] = pixel_values

        # --- 处理指令 ---
        # obs['instruction'] 已经是 tokenized 的整数数组 (由环境生成)
        input_ids = torch.from_numpy(obs['instruction']).long()  # [T, seq_len] 或 [T]（取决于 tokenizer）
        obs['input_ids'] = input_ids
        if self.tokenizer.pad_token_id is not None:
            attention_mask = (input_ids != self.tokenizer.pad_token_id).long()
            obs['attention_mask'] = attention_mask
...
def collect_data(data_it=0):
    ...
    # 初始化训练器，但传入自定义策略
    trainer = initialize_trainer()  # 假设 initialize_trainer 已按之前讨论修改，返回的 trainer 包含新模型

    if torch.cuda.is_available():
        with torch.cuda.device(trainer.device):
            torch.cuda.empty_cache()

    # 移除所有特征提取钩子，因为新模型不使用中间特征
    # 也可以设置 args.ablate_rgb 和 args.ablate_depth 为 True 来跳过钩子注册

    # 如果原 trainer 初始化时注册了钩子，可以在这里移除或跳过
    # 示例：确保不注册钩子（可在 initialize_trainer 中避免）
    # 或者直接注释掉钩子相关代码

    # 混合专家概率（可调整为需要的值）
    p = 1.0
    beta = 1.0  # 可调节：1.0 表示全部使用专家动作，0.0 表示全部使用策略动作
    ...
    with torch.no_grad():
        ...
        while train_env.index_data < end_iter:
            ...
            # 初始化 RNN 状态（如果新模型不需要，可以忽略，但需要占位）
            rnn_states = torch.zeros(
                train_env.batch_size,
                1,  # 原代码中可能为 num_recurrent_layers，如果你的新模型不需要，可以设为 0 或 dummy
                1,
                device=trainer.device,
            ) if hasattr(trainer.policy, 'net') else None
            # 如果新模型不需要 RNN，将 rnn_states 设置为 None，并确保 act 能处理 None
            ...
            batch = batch_obs(observations, trainer.device)  # 仍使用原始 batch_obs，但注意 batch 中不包含预处理特征
            batch = preprocess_batch(batch, trainer)  # 新增预处理
            ...
            for t in range(int(args.maxAction) + 1):
                ...
                # 混合专家动作（beta 概率使用专家动作）
                # teacher_actions = batch['teacher_action'].long().unsqueeze(1)  # [batch_size, 1]
                teacher_actions = batch['teacher_action'].long()  # [batch_size, 1]
                actions = torch.where(
                ...
                # 记录当前步的观测（原始观测）、上一步动作、专家动作
                for i in range(train_env.batch_size):
                    if i in envs_to_pause:
                        continue
                    episodes[i].append(
                        (
                            observations[i],              # 原始观测（包含 rgb, instruction 等）
                            prev_actions[i].item(),       # 实际执行的动作
                            batch['teacher_action'][i].item(),  # 专家动作
                        )
                    )
            ...
            for i in range(train_env.batch_size):
                ...
                del traj_obs['teacher_action']  # 可选，可根据需要保留
                ...
                try:
                    ...
                    data = custom_msgpack.packb(transposed_ep)  # 使用自定义 packb
...
def preprocess_batch(batch, trainer):
    """
    batch: dict of tensors, each with shape [batch, ...] (可能包含多个时间步的展平)
    为 batch 添加 pixel_values, input_ids, attention_mask
    """
    image_processor = trainer.policy.image_processor
    tokenizer = trainer.policy.tokenizer

    # 处理图像
    rgb = batch['rgb']  # [N, H, W, 3] or [total_steps, H, W, 3]
    # 确保在 CPU 且为 uint8
    rgb_np = rgb.cpu().numpy().astype(np.uint8)
    # 转换为 PIL 列表
    from PIL import Image
    # 如果 rgb_np 是 4D，则第一维是 batch/步数
    if rgb_np.ndim == 4:
        pil_images = [Image.fromarray(img) for img in rgb_np]
    else:
        # 单张图像，包装为列表
        pil_images = [Image.fromarray(rgb_np)]
    pixel_values = image_processor(pil_images, return_tensors='pt')['pixel_values'].to(trainer.device)
    batch['pixel_values'] = pixel_values

    # 处理指令
    input_ids = batch['instruction']  # [N, seq_len] 或 [N]
...
def train_vlnce():
    ...
    # 如果使用 deepspeed，用 deepspeed.initialize 包装模型
    if args.deepspeed:
        # 获取可训练参数（只训练 LoRA 和动作头）
        # model_parameters = filter(lambda p: p.requires_grad, trainer.policy.parameters())
        ...
        trainer.model_engine, trainer.optimizer, _, _ = deepspeed.initialize(
            args=args,  # 传递命令行参数，其中包含 --deepspeed_config 信息
    ...
    for dagger_it in range(int(args.dagger_it)):
        ...
        for epoch in tqdm.trange(int(args.epochs), dynamic_ncols=True):
            ...
            if is_main_process():
                save_dir = Path(args.project_prefix) / 'DATA/output/{}/train/checkpoint/{}'.format(args.name,
                                                                                                   args.make_dir_time)
                ...
                if args.deepspeed:
                    ...
                    # trainer.save_checkpoint(f"ckpt.{dagger_it}.{epoch}.pth", dagger_it, epoch)
...
def _eval_checkpoint(
    ...
    with torch.no_grad():
        ...
        for idx in range(start_iter, end_iter, train_env.batch_size):
            ...
            if args.policy_type in ['seq2seq', 'cma'] and hasattr(trainer.policy, 'net'):
                rnn_states = torch.zeros(
                    train_env.batch_size,
                ...
                # 新策略：没有 RNN 层，rnn_states 设为 None
                rnn_states = None
            prev_actions = torch.zeros(
                train_env.batch_size,
                1,
