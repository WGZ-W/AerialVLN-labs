...
class PaddedCollatorForActionPrediction:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        input_ids, labels = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels"))
        pixel_values = [instance["pixel_values"] for instance in instances]
        history_values = [instance["history_values"] for instance in instances]
        # cur_imgs = [instance["cur_img"] for instance in instances]
        # language = [instance["language"] for instance in instances]
        ...
        # Stack all `cur_img` --> depending on type (torch.Tensor, or Dict[str, torch.Tensor]) & presence of None
        # if isinstance(cur_imgs[0], torch.Tensor):
        ...
        if isinstance(history_values[0], torch.Tensor):
            history_values = torch.stack(history_values)
...
def pprint_data_mixture(dataset_kwargs_list: List[Dict[str, Any]], dataset_weights: List[int]) -> None:
    print("\n######################################################################################")
    print(f"# Loading the following {len(dataset_kwargs_list)} dataset (incl. sampling weight):{'': >24} #")
...
def get_dataset_statistics(
    ...
    if cardinality == tf.data.INFINITE_CARDINALITY:
        raise ValueError("Cannot compute dataset statistics for infinite dataset.")
...
def allocate_threads(n: Optional[int], weights: np.ndarray):
    """
    Allocates an integer number of threads across dataset based on weights.
...
def decode_and_resize(
    ...
    # 特殊处理 history_images
    history_images_keys = {key for key in obs if key == "history_images"}
    ...
    # 处理 history_images 张量
    for key in history_images_keys:
        images = obs[key]

        # 检查是否是字符串编码的张量
        if images.dtype == tf.string:
            # 检查形状以确定是否是多图像张量
            if len(images.shape) == 1:  # [T] 每个元素是一个字符串
                def decode_single_img(img_str):
                    if tf.strings.length(img_str) == 0:
                        # 创建填充图像
                        default_size = resize_size.get("history", (1, 1))
                        return tf.zeros((*default_size, 3), dtype=tf.uint8)
                    else:
                        return tf.io.decode_image(img_str, expand_animations=False, dtype=tf.uint8)
                ...
                raise ValueError(f"history_images with dtype string should have shape [T], got {images.shape}")

        # 调整大小（如果需要）
        resize_key = "history_images" if "history_images" in resize_size else "history"
        if resize_key in resize_size:
            target_size = resize_size[resize_key]

            # 对每个时间步调整大小
            def resize_single_img(img):
                return dl.transforms.resize_image(img, size=target_size)

            images = tf.map_fn(
                resize_single_img,
                images,
                fn_output_signature=tf.TensorSpec(shape=(*target_size, 3), dtype=tf.uint8)
            )
