...
# 用于在训练输出目录中查找最新的检查点路径，主要用于模型训练过程中的断点续训。
def get_checkpoint_path(output_dir: str, checkpoint_prefix: str = "checkpoint") -> Optional[str]:
...
# 用于为多模态模型训练准备配置信息，主要作用是将各种参数源（模型参数、训练参数、数据参数）整合到统一的配置对象中。
...
def get_siglip_transform(image_processor):
    """根据 SiglipImageProcessor 创建 torchvision transform"""
    transforms = []

    # 调整大小
    if image_processor.do_resize:
        size = image_processor.size
        height = size.get('height', size) if isinstance(size, dict) else size
        transforms.append(T.Resize((height, height), interpolation=InterpolationMode.BICUBIC))
    ...
    if image_processor.do_rescale:
        # SigLIP 通常使用 1/255 的重缩放因子
        transforms.append(T.Lambda(lambda x: x * image_processor.rescale_factor))
