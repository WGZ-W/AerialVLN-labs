...
from llava.model.multimodal_projector.builder import build_mb_projector
from llava.model.history_mamba.builder import build_history_mamba
...
class LlavaMetaModel(ABC):
    def init_vlm(self, config: PreTrainedModel = None, *args, **kwargs):
        ...
        self.vision_tower = build_vision_tower(vision_tower_cfg, config)
        self.mm_projector = build_mm_projector(mm_projector_cfg, config)
        if os.path.exists(os.path.join(config.resume_path, 'history_mamba')):
            history_mamba_path = os.path.join(config.resume_path, 'history_mamba')
            if os.path.exists(os.path.join(history_mamba_path, 'history_mamba.pth')):
                history_mamba_path = os.path.join(history_mamba_path, 'history_mamba.pth')
        else:
            history_mamba_path = None

        self.history_mamba = build_history_mamba(config, history_mamba_path)
        self.mb_projector = build_mb_projector(config)
    ...
    def init_vlm2(self, config: PreTrainedModel = None, *args, **kwargs):
        # TODO(ligeng): figure out how from_config and from_pretrained works in HF implementation.
        if hasattr(self, "llm") or hasattr(self, "vision_tower") or hasattr(self, "mm_projector"):
            # already initialized, skipped
            return
    ...
    def save_pretrained(self, output_dir, state_dict=None):
        ...
        if self.get_history_mamba() is not None:
            print(f"saving history_mamba to {osp.join(output_dir, 'history_mamba')}")
            # self.history_mamba.config._name_or_path = osp.join(output_dir, "history_mamba")
            history_mamba_state_dict = OrderedDict(
                {k.split("history_mamba.")[-1]: v for k, v in state_dict.items() if "history_mamba" in k})
            # self.history_mamba.save_pretrained(os.path.join(output_dir, "history_mamba"),
            #                                    state_dict=history_mamba_state_dict)
            history_mamba_path = osp.join(output_dir, "history_mamba")
            os.makedirs(history_mamba_path, exist_ok=True)
            torch.save(history_mamba_state_dict, osp.join(history_mamba_path, "history_mamba.pth"))
            # self.config.history_mamba_cfg = self.history_mamba.config
        ...
        # if self.get_mb_projector():
        #     print(f"saving mb_projector to {osp.join(output_dir, 'mb_projector')}")
        #     self.mm_projector.config._name_or_path = osp.join(output_dir, "mb_projector")
        #     mm_projector_state_dict = OrderedDict(
        #         {k.split("mb_projector.")[-1]: v for k, v in state_dict.items() if "mb_projector" in k}
        #     )
        #     self.mb_projector.save_pretrained(
        #         os.path.join(output_dir, "mb_projector"),
        #         state_dict=mm_projector_state_dict,
        #     )
        #     self.config.mb_projector_cfg = self.mb_projector.config
    ...
    def get_mb_projector(self):
        mb_projector = getattr(self, "mb_projector", None)
        if type(mb_projector) is list:
            mb_projector = mb_projector[0]
        return mb_projector
    ...
    def encode_images(self, images):
        print(f"encode_images input shape: {images.shape}")  # 应该是 [1, 3, 384, 384]

        image_features = self.get_vision_tower()(images)
        print(f"vision tower output shape: {image_features.shape}")
        image_features = self.get_mm_projector()(image_features)
        print(f"mm projector output shape: {image_features.shape}")
...
class LlavaMetaForCausalLM(ABC):
    ...
    @torch.inference_mode()
    # def generate(self, input_ids, images, **kwargs: str) -> str:
    #     # For now, only support generation with a batch size of 1 for simplicity
    #
    #     if isinstance(images, torch.Tensor):
    #         pixel_values = images[None, ...].to(self.device)
    #     else:
    #         raise ValueError(f"Unsupported `pixel_values` type = {type(images)}")
    #
    #     # Invoke super().generate --> taps into `GenerationMixin` which (redirects) to `forward()`
    #     # autocast_dtype = self.llm.half_precision_dtype
