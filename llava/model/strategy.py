...
from llava.model.prismatic import PrismaticVLM
from llava.model.overwatch import initialize_overwatch
from llava.model.metrics import Metrics, VLAMetrics
from llava.dataset.data_utils import SplitModalitySampler
from llava.dataset.data_utils import PaddedCollatorForActionPrediction, PaddedCollatorForLanguageModeling
...
class TrainingStrategy(ABC):
    def __init__(
        ...
        **_: str,
        ...
        self.llm_transformer_layer_cls = LlamaDecoderLayer
    ...
    def save_checkpoint(
            ...
            with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy):
                ...
                if overwatch.is_rank_zero():
                    ...
                    checkpoint_dir.mkdir(parents=True, exist_ok=True)  # 确保目录存在
    ...
    def run_vla_training(
        ...
        with tqdm(
            ...
            for batch in dataloader:
                ...
                # 加入 Mamba 之后，使用最后的作为输出
                action_size = batch["labels"].shape[1]
                action_preds = output.logits[:, -action_size: -1].argmax(dim=2)
                ...
                if (terminate := (self.max_steps is not None and metrics.global_step >= self.max_steps)) or (
                    ...
                    # )
                    self.vlm.config.save_pretrained(str(metrics.run_dir))
                    self.vlm.save_pretrained(str(metrics.run_dir), state_dict=self.vlm.state_dict())
                    dist.barrier()
